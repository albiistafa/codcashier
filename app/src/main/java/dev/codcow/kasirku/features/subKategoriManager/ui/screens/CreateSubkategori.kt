...
fun CreateSubKategori(
    ...
    // In your CreateSubKategori composable, change this line:
    ...
    Scaffold(
        ...
        Column(
            ...
            Button(
                onClick = {
                    if (namaKategori.isNotBlank() && selectedCategoryId != null) {
                        scope.launch {
                            val request = CreateSubkategoriRequest(
                                name = namaKategori.trim(),
                                category_id = selectedCategoryId!!
                            )
                            // The function doesn't return a Result, it handles success/failure internally
                            viewModel.createSubKategori(request)
                            // No need to check result here since the ViewModel will update states
                            // that we're already observing
                        }
                    }
                },
                modifier = Modifier
